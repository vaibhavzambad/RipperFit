...
import resource.OAuthConstants;
import resource.OAuthDetails;
import resource.OAuthUtils;

/**
 * Servlet implementation class OAuth2Client
 */
@WebServlet("/OAuth2Client")
public class OAuth2Client extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		OAuthDetails oauthDetails = OAuthUtils.createOAuthDetails(config);
		...
		if(invalidProps!=null && invalidProps.size() == 0){
			...
			if(OAuthUtils.isValid(caller)){
				...
				if(caller.equalsIgnoreCase(OAuthConstants.TOKEN)){
					...
					if(!OAuthUtils.isValid(oauthDetails.getResource_server_url())){
						invalidProps.add(OAuthConstants.RESOURCE_SERVER_URL);

					}

					if(!OAuthUtils.isValid(oauthDetails.getAccess_token())){
						if(!OAuthUtils.isValid(oauthDetails.getRefresh_token())){
