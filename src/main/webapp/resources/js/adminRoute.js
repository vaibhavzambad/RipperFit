var app = angular
		...
		.factory(
				'StoreCurrentLoggedInUserInformationService',
				[
						"$window",
						function($window) {
							function set(data) {
						...
						headers : {
							...
							.then(
									function(response) {
										...
										$scope.loggedEmployee = $scope.employee.firstName
												+ " "
												+ $scope.employee.lastName;
										if ($scope.employee.designation != null) {
											var str = $filter('uppercase')
													(
		...
		.controller(
				...
				function($scope, $http, $routeParams, $window, $filter) {
							$http
									.get(
											"/RipperFit/request/getRequest/"
													+ $routeParams.requestId)
									.then(
											function(response) {
												$scope.request = response.data;
												$scope.request.requestDate = $filter(
														'date')
														(
																new Date(
																		$scope.request.requestDate),
																'yyyy-MM-dd');
												$scope.UpdateData = function(
													...
													for (var i = 0; i < length; i++) {
														$scope.req[i].requestDate = $filter(
																'date')
																(
																		new Date(
																				$scope.req[i].requestDate),
																		'yyyy-MM-dd');
													}
													$http(
															{
															...
															.then(
																	function() {
																		$window.location.href = "#/viewOwnRequests/"
																				+ $scope.request.employee.employeeId;
																	},
																	function(
																			response) {
																		console
		...
		.controller(
				...
				function($scope, $http, $filter,
					...
					$scope.employee = StoreCurrentLoggedInUserInformationService
							.get();
							$http
									.get(
											"/RipperFit/request/getRequestsByOrganization/"
													+ $scope.employee.organization.organizationId)
									.then(
											function(response) {
												...
												if (length == 0) {
													...
													for (var i = 0; i < length; i++) {
														$scope.requests[i].requestDate = $filter(
																'date')
																(
																		new Date(
																				$scope.requests[i].requestDate),
																		'yyyy-MM-dd');
														if ($scope.requests[i].status == "completed"
																|| $scope.requests[i].status == "success") {
		...
		.controller(
				...
				function($scope, $http,
					...
					$scope.employee = StoreCurrentLoggedInUserInformationService
							...
							$http
									.get(
											"/RipperFit/designation/getDesignations/"
													+ $scope.employee.organization.organizationId)
		...
		.controller(
				...
				function($scope, $http,
						StoreCurrentLoggedInUserInformationService) {
					$scope.employeeDetail = StoreCurrentLoggedInUserInformationService
							.get();
							$http
									.get(
											"/RipperFit/employee/getEmployeesByOrganizationId/"
													+ $scope.employeeDetail.organization.organizationId)
									.then(function(response) {
										$scope.employee = response.data;
									}), function(response) {
								console.log(response.status);
		...
		.controller(
				...
				function($scope, $http, $routeParams) {
							$http
									.get("/RipperFit/request/getAllRequests")
									.then(
											function(response) {
												$scope.requests = response.data;
												var length = $scope.requests.length;
		...
		.controller(
				...
				function($scope, $http,
					...
					$scope.employee = StoreCurrentLoggedInUserInformationService
							...
							$http
									.get(
											"/RipperFit/departments/getAllDepartmentsInAnOrganization/"
													+ $scope.employee.organization.organizationId)
									.then(function(response) {
		...
		.controller(
				...
				function($scope, $http, $window) {
					$scope.employeeList = "";
					$http
							.get("/RipperFit/employee/getEmployeeApprove")
							.then(
									function(response) {
										...
										var myEl = angular.element(document
												.querySelector('#table'));
										var myE2 = angular.element(document
												.querySelector('#msg'));
										if (length == 0) {
											myEl.addClass('hidden');
											myE2.removeClass('hidden');
		...
		.controller(
				"viewEmployeeRequestsCtrl",
				function($scope, $http, $routeParams, $window) {
							$http
									.get(
											"/RipperFit/request/getRequestByEmployee/"
		...
		.controller(
				...
				function($scope, $http, $routeParams, $window) {
					$http
							.get(
									"/RipperFit/departments/getDepartmentById/"
				...
				function($scope, $http, $filter,
					...
					if (des == "HELPDESK") {
						...
						myEl.addClass('hidden');
							...
							$http
									.get(
											"/RipperFit/resource/getResourcesByOrganizationId/"
													+ $scope.employee.organization.organizationId)
									.then(
											function(response) {
							...
							$http
									...
									.then(
											function(response) {
												...
												$http
														.get(
																"/RipperFit/comment/getCommentByRequestId/"
																		+ $routeParams.requestId)
														.then(
																function(
																		response) {
																	...
																	for (var i = 0; i < length; i++) {
																		$scope.comments[i].date = $filter(
																				'date')
																				(
																						new Date(
																								$scope.comments[i].date),
																						'yyyy-MM-dd');
																	}
																})
											}), function(response) {
				...
				function($scope, $http, $window, $filter) {
					...
					$scope.completeRequest = function(request) {
						...
						$http(
								{
									method : 'GET',
									url : "/RipperFit/approve/completeRequest/"
											+ request.requestId,
									headers : {
										'Content-Type' : 'application/json'
									}
					...
					$scope.rejectRequest = function(request) {
						...
						$http(
								{
									method : 'GET',
									url : "/RipperFit/approve/rejectRequest/"
											+ request.requestId,
									headers : {
										'Content-Type' : 'application/json'
									}
		...
		.controller(
				...
				function($scope, $http, $window,
					...
					$scope.employeeDetails = StoreCurrentLoggedInUserInformationService
							.get();

					$http
							.get(
									"/RipperFit/employee/getEmployeesByOrganizationId/"
											+ $scope.employeeDetails.organization.organizationId)
							.then(
									function(response) {
										$scope.employeeList = response.data;
										$scope.newList;
										...
										for (var i = 0; i < $scope.employeeList.length;) {
											...
											if ($scope.employeeDetails.email == $scope.employeeList[i].email
													|| $scope.employeeList[i].designation.designationName == "Helpdesk"
													|| $scope.employeeList[i].designation.designationName == "Admin") {
												$scope.employeeList
														.splice(i, 1);
												k++;
											} else {
					...
					$scope.reportId = function(emp) {
						...
						$http(
								{
									method : 'GET',
									url : "/RipperFit/employee/getEmployeeByEmail?email="
											+ email,
									headers : {
										'Content-Type' : 'application/json'
									}
					...
					$scope.UpdateData = function(employeeDetails) {
						$scope.employee = {
							...
							"password" : $scope.employeeDetails.password,
				...
				function($scope, $http, $window, $filter,
					...
					$scope.getResources = function() {
						$http(
								{
									method : 'GET',
									url : "/RipperFit/resource/getResourcesByOrganizationId/"
											+ $scope.employee.organization.organizationId,
									headers : {
										'Content-Type' : 'application/json'
									}
						...
						$http({
							...
							headers : {
								...
								.then(
										function() {
											$window.location.href = "#/viewOwnRequests/"
													+ $scope.requestDetails.employee.employeeId;
										}, function(response) {
											console.log(response.status);
										});
					}
				...
				function($scope, $http, $filter,
					...
					$scope.employee = StoreCurrentLoggedInUserInformationService
							...
							$http(
									{
										method : 'GET',
										url : "/RipperFit/request/getAllRequestToApprove/"
												+ $scope.employee.employeeId,
										headers : {
											'Content-Type' : 'application/json'
										}
									...
									.then(
											function(response) {
												...
												if (length == 0) {
													...
													myE2.addClass('hidden');
													for (var i = 0; i < length; i++) {
														$scope.requestsToApprove[i].resourceRequest.requestDate = $filter(
																'date')
																(
																		new Date(
																				$scope.requestsToApprove[i].resourceRequest.requestDate),
																		'yyyy-MM-dd');
														if ($scope.requestsToApprove[i].resourceRequest.status == "completed"
																|| $scope.requestsToApprove[i].resourceRequest.status == "success") {
				...
				function($scope, $http, $window) {
					$scope.forwardRequest = function(request) {
								$http(
										{
											method : 'GET',
											url : "/RipperFit/approve/forwardRequest/"
													+ request.resourceRequest.requestId,
											headers : {
												'Content-Type' : 'application/json'
											}
										})
				...
				function($scope, $http, $window, $filter,
					...
					$scope.getDepartments = function() {
						$http(
								{
									method : 'GET',
									url : "/RipperFit/departments/getAllDepartmentsInAnOrganization/"
											+ $scope.employee.organization.organizationId,
									headers : {
										'Content-Type' : 'application/json'
									}
					...
					$scope.getDesignations = function(department) {
						$http(
								{
									method : 'GET',
									url : "/RipperFit/designation/getDesignationsByDepartment/"
					...
					$scope.getDesignationDetails = function(position) {
						...
						$http
								.get(
										"/RipperFit/employee/getCurrentEmployeeObject")
								.then(
										function(response) {
		...
		.controller(
				...
				function($scope, $http, $routeParams, $window) {
					$http
							.get(
									"/RipperFit/resource/getResourceById/"
		...
		.controller(
				...
				function($scope, $http, $window,
					...
					$scope.changePassword = function(user) {
						var message = angular.element(document
								.querySelector('#message'));
						if (user.new_password == user.confirm_password) {
							$http(
									{
										method : 'POST',
										url : "/RipperFit/employee/changePassword?oldPassword="
												+ user.old_password
												+ "&newPassword="
												+ user.new_password,
										headers : {
		...
		.controller(
				...
				function($scope, $http, $routeParams, $window,$filter
					...
					$http.get(
							"/RipperFit/request/getRequestByEmployee/"
									+ $scope.employee.employeeId + " ").then(
							function(response) {
								$scope.request = response.data;
					...
					$http.get(
							"/RipperFit/request/getRequestByStatus/"
					...
					$http.get(
							"/RipperFit/request/getRequestByStatus/"
									+ $scope.running).then(function(response) {
					...
					$http.get(
							"/RipperFit/request/getRequestByStatus/"
									+ $scope.rejected).then(function(response) {
						$scope.totalRejectedRequests = response.data.length;
					});
				})
/*
 * .controller("pendingRequestsCtrl", function($scope, $http,
 * StoreCurrentLoggedInUserInformationService) { $scope.pending="pending";
